...
const EventCard: React.FC<EventCardProps> = ({ event, onRegister }) => {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-[24px] bg-white/40 backdrop-blur-md shadow-sm transition-all hover:shadow-xl border border-white/20">
      ...
      <Link to={`/evento/${event.id}`} className="aspect-[16/9] overflow-hidden relative block">
        <img
          ...
          loading="lazy"
        />
        <div aria-hidden="true" className="absolute top-4 left-4 right-4 flex justify-between items-start pointer-events-none">
        ...
        <div aria-hidden="true" className="absolute bottom-4 right-4 bg-white/90 backdrop-blur-md rounded-xl px-3 py-2 shadow-lg pointer-events-none">
      ...
      <div className="p-6">
        ...
        <h3 className="mb-3 text-lg font-black text-slate-900 group-hover:text-primary transition-colors leading-tight tracking-tight">
          {event.title}
        </h3>
